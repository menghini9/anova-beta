"use client";
// ======================================================
// ChatSidePanels — ChatGPT-like Sidebar (Collapsible, Clean Header)
...
export default function ChatSidePanels(props: Props) {
  ...
  return (
    ...
    >
      {/* Wrapper contenuti (evita click quando chiusa) */}
      <div className={sidebarOpen ? "w-[340px]" : "w-[340px] pointer-events-none"}>
        {/* ======================================================
            HEADER — 2 gruppi coerenti: (Archivio/Cestino) + (New/Close)
           ====================================================== */}
        <div className="h-16 px-4 flex items-center justify-between border-b border-white/10">
          {/* LEFT GROUP: Archivio / Cestino */}
          ...
          {/* RIGHT GROUP: + (New) / X (Close) */}
          <div className="flex items-center gap-3">
            <button
              onClick={onNewSession}
              className="h-10 w-10 rounded-xl border border-white/15 bg-white/10 text-[18px] font-semibold text-white hover:bg-white/14 hover:border-white/30 transition flex items-center justify-center"
              title="Nuova chat"
            >
              +
            </button>
